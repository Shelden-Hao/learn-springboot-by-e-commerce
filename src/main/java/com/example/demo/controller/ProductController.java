...
public class ProductController {
    ...
    public StockCheckResponse checkStock(@RequestParam Long qty, @PathVariable Long id) {
        Product product = productMapper.selectById(id);
        ...
        // 库存不足 → available: false，不是抛异常
        if (product.getStock() < qty) {
            return new StockCheckResponse(product.getId(), product.getName(), qty, false, product.getStock());
        }
        // 库存充足 → available: true
        return new StockCheckResponse(product.getId(), product.getName(), qty, true, product.getStock());
    }
